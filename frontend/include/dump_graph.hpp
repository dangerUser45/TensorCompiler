            ...
            switch (init.get_data_type().id) {
                case DataID::INT8:
                    return print_vector_multiline_html(
                        init.get_values<int8_t>());
                case DataID::INT16:
                    return print_vector_multiline_html(
                        init.get_values<int16_t>());
                case DataID::INT32:
                    return print_vector_multiline_html(
                        init.get_values<int32_t>());
                case DataID::INT64:
                    return print_vector_multiline_html(
                        init.get_values<int64_t>());
                case DataID::UNSIGNED_INT8:
                    return print_vector_multiline_html(
                        init.get_values<uint8_t>());
                case DataID::UNSIGNED_INT16:
                    return print_vector_multiline_html(
                        init.get_values<uint16_t>());
                case DataID::UNSIGNED_INT32:
                    return print_vector_multiline_html(
                        init.get_values<uint32_t>());
                case DataID::UNSIGNED_INT64:
                    return print_vector_multiline_html(
                        init.get_values<uint64_t>());
                case DataID::FLOAT:
                    return print_vector_multiline_html(
                        init.get_values<float>());
                case DataID::DOUBLE:
                    return print_vector_multiline_html(
                        init.get_values<double>());
                case DataID::STRING:
                    return print_vector_multiline_html(
                          ...
                          const std::string& title)
    {
        for (size_t i = 0; i != tensors.size(); ++i) {
            ...
            out_ << "<TR><TD BGCOLOR=\"" << IO_HEADER_COLOR << "\"><B>"
                 ...
                 << tensors[i]->get_data_type().data_type_str
                 << "</TD></TR>\n\t\t\t\t"
                    "<TR><TD BGCOLOR=\""
                 << IO_BASE_COLOR
        ...
        for (size_t i = 0; i != inits.size(); ++i) {
            ...
            out_ << "<TR><TD BGCOLOR=\"" << INITS_HEADER_COLOR
                 << "\"><B>INITIALIZER</B></TD></TR>\n\t\t\t\t"
                    "<TR><TD BGCOLOR=\""
                 << INITS_BASE_COLOR
                 << "\"><B>name</B>: " << inits[i]->get_name()
                 << "</TD></TR>\n\t\t\t\t"
                    "<TR><TD BGCOLOR=\""
                 << INITS_BASE_COLOR << "\"><B>dtype</B>: "
                 << inits[i]->get_data_type().data_type_str
                 << "</TD></TR>\n\t\t\t\t"
                    "<TR><TD BGCOLOR=\""
                 << INITS_BASE_COLOR
                   ...
                   const Graph::TensVecT& outputs)
    {
        ...
        for (size_t i = 0; i != inputs.size(); ++i) {
            ...
            const std::string input_name =
                normalize_name(inputs[i]->get_name());
        ...
        for (size_t i = 0; i != outputs.size(); ++i) {
            ...
            const std::string output_name =
                normalize_name(outputs[i]->get_name());
