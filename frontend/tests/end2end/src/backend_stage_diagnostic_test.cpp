...
TEST(BackendStageDiagnostic, EmitMlirFailureReportsBackendStage)
{
    ...
    const std::filesystem::path tmp_output =
        std::filesystem::temp_directory_path() /
        ("tc_backend_diag_" + std::to_string(getpid()) + ".log");
    ...
    EXPECT_NE(exit_code, 0) << "expected emit-mlir failure";
    EXPECT_NE(tc::frontend::testutil::ToLower(output).find("backend"),
              std::string::npos)
